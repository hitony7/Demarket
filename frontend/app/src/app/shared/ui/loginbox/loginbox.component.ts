...
export class LoginBoxComponent {
  ...
  async connectWallet(walletName: string) {
    if (walletName === 'MetaMask') {
      ...
      alert(`${walletName} integration is not yet available.`);
    }
  }
 ...
 async connectMetaMask() {
  ...
  async authenticateWithBackend(signer: ethers.Signer, address: string) {
    try {
      ...
      const nonceResponse = await this.authService.requestNonce(address);
      ...
      const tokenResponse = await this.authService.verifySignature(address, signature);
