...
export class AuthService {
  ...
  async requestNonce(walletAddress: string): Promise<NonceResponse> {
    return lastValueFrom(
      this.http.post<NonceResponse>(`${environment.apiBaseUrl}/api/auth/request-nonce`, { walletAddress })
    );
  }
   ...
   */
  async verifySignature(walletAddress: string, signature: string): Promise<TokenResponse> {
    return lastValueFrom(
      this.http.post<TokenResponse>(`${environment.apiBaseUrl}/api/auth/verify-signature`, { walletAddress, signature })
    );
  }
